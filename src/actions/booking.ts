...
export async function updateAvailabilities(data: Partial<Availability>[]) {
	...
	return await db.transaction(async (trx) => {
		for (const d of data) {
			if (!d.day) continue;
			await trx.update(availability).set(d).where(eq(availability.day, d.day));
		}
	});
}
...
export async function createOrUpdateBlockedSlot(
	data: Partial<BlockedAvailability>[],
) {
	const session = await auth.api.getSession({
		headers: await headers(),
	});
...
export async function deleteAllBlockedSlotsAndAddNew(
	data: Partial<BlockedAvailability>[],
) {
	const session = await auth.api.getSession({
		headers: await headers(),
	...
	return await db.transaction(async (trx) => {
		...
		for (const d of data) {
			if (!d.date || !d.slots) continue;
			await trx.insert(blockedAvailability).values({
				date: d.date,
				slots: d.slots,
			});
		}
...
export async function getNext30DaysAvailableDays() {
	...
	const result = next30Days.map((date) => {
		...
		const blockedDate = blockedDates.find((b) => {
			...
			return (
				blockedDateTime.toISOString().split("T")[0] ===
		...
		if (blockedDate) {
			...
			availableSlots = daySchedule.slots.filter(
				(slot) => !blockedDate.slots.includes(slot),
			);
		}
		...
		const bookedSlots = fixedConfirmedBooking
			.filter((b) => {
				return (
...
// Reshedule booking
export async function resheduleBooking({
	id,
	date,
	startingSlot,
}: {
	...
	const currentBooking = (
		await db.select().from(booking).where(eq(booking.id, id)).limit(1)
	)[0];
	...
	return await db
		...
		.where(
			and(
				...
				or(
					and(eq(booking.status, "confirmed"), lte(booking.time, new Date())),
...
export async function createRazorpayOrder(data: FormDataType) {
	...
	const product =
		parsedData.productType === "service"
			? services[parsedData.productId]
			: packages[parsedData.productId];
	...
	for (let i = 0; i < parsedData.productCount; i++) {
		newBookings.push({
			...
			slots:
				i === 0
					? [
							parsedData.startingSlot,
							parsedData.startingSlot + 1,
							parsedData.startingSlot + 2,
							parsedData.startingSlot + 3,
						]
					: null,
			time:
				i === 0
					? convertToDate(parsedData.date, parsedData.startingSlot)
					: null,
...
export async function checkIfSlotAvailible(date: Date, startingSlot: number) {
	...
	requestedSlotsArray.forEach((requestedSlots) => {
		const availabileSlots = availabilities.find(
			(availability) =>
				availability.date.toISOString().split("T")[0] ===
				requestedSlots.date.toISOString().split("T")[0],
		)?.slots;
		if (!availabileSlots) {
			throw new Error("No slots available for this date");
		}
