...
export function EditQuiz({ id }: { id: string }) {
    ...
    const { data: quiz } = useQuery({ queryKey: ["quizzes", id], queryFn: () => getQuizById(id) });
    // Form Schema
                ...
                <Form {...form}>
                    <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                        ...
                        <Button type="submit" className="w-full" disabled={mutation.isPending || pendingUploads.size > 0}>
                            {mutation.isPending ? "Saving Changes..." : pendingUploads.size > 0 ? "Waiting for uploads..." : "Save Quiz"}
                        </Button>
                    </form>
                </Form>
            </DialogContent>
